...
class QuizAction extends AbstractDatabaseObjectAction implements IToggleAction, IGroupedUserListAction, IUploadAction {
	...
	public function getQuiz() {
		...
		foreach ($questionIDs as $id) {
			...
			if (!empty($question->answerThree)) $answers[] = StringUtil::encodeHTML(WCF::getLanguage()->get($question->answerThree));
			if (!empty($question->answerFour)) $answers[] = StringUtil::encodeHTML(WCF::getLanguage()->get($question->answerFour));
			if (!empty($question->answerFive)) $answers[] = StringUtil::encodeHTML(WCF::getLanguage()->get($question->answerFive));
			if (!empty($question->answerSix)) $answers[] = StringUtil::encodeHTML(WCF::getLanguage()->get($question->answerSix));
			
			$max = 0;
			foreach ($answers as $answer) {
				$temp = explode(' ', $answer);
				$maxThis = max(array_map('strlen', $temp));
				if ($max < $maxThis) $max = $maxThis;
			}
	...
	public function saveResult() {
		...
		if ($this->quiz->showCorrect) {
			...
			if ($points < 11) $comment = WCF::getLanguage()->get('wcf.user.quiz.comment.zeroToTen');
			else if ($points < 36) $comment = WCF::getLanguage()->get('wcf.user.quiz.comment.tenToThirtyfive');
			else if ($points < 66) $comment = WCF::getLanguage()->get('wcf.user.quiz.comment.thirtyfiveToSixtyfive');
			else if ($points < 91) $comment = WCF::getLanguage()->get('wcf.user.quiz.comment.sixtyfiveToNinety');
			else if ($points < 100) $comment = WCF::getLanguage()->get('wcf.user.quiz.comment.ninetyToNinetynine');
			else $comment = WCF::getLanguage()->get('wcf.user.quiz.comment.hundert');
		}
		else {
	...
	public function getStats() {
		...
		$averageRate = 0;
		if ($row['total']) $averageRate = $row['correct'] / $row['total'] * 100;
		$userCount = $row['users'];
		...
		$maxRate = $minRate = 0;
		if ($row['correctMax'] <= $total) $correctMax = $row['correctMax'];
		if ($row['correctMin'] <= $total) $correctMin = $row['correctMin'];
		if ($total > 0) {
	...
	public function getAllowedGroupIDs($groupIDs) {
		...
		foreach (UserGroup::getGroupsByIDs($groupIDs) as $group) {
			if (!$group->isAdminGroup()) $allowedUserGroupIDs[] = $group->groupID;
		}
