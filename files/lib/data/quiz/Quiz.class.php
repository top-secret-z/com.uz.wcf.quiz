...
class Quiz extends DatabaseObject implements IRouteController
{
    ...
    public function hasSolved()
    {
        ...
        if (\in_array($this->quizID, $played)) {
            return true;
        }
    ...
    public function canSee()
    {
        ...
        // all for admin
        if ($session->getPermission('admin.community.canManageQuiz')) {
            return true;
        }
        ...
        if (!$session->getPermission('user.quiz.canSee')) {
            return false;
        }

        // period
        if ($this->hasPeriod) {
            if ($this->periodStart > TIME_NOW || $this->periodEnd < TIME_NOW) {
                return false;
            }
        }
    ...
    {
        // permission / guest
        if (!WCF::getSession()->getPermission('user.quiz.canPlay')) {
            return false;
        }
        ...
        // play again
        if (!$this->playAgain && $this->hasSolved()) {
            return false;
        }
    ...
    {
        if (!QUIZ_GROUP_ON || !QUIZ_FILTER_SUCCESS) {
            return true;
        }
        if (!$this->playAgain || empty(\unserialize($this->assignGroupIDs))) {
            return true;
        }
