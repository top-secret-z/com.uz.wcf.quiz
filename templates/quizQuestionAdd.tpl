{capture assign='pageTitle'}{lang}wcf.acp.quiz.question.{@$action}{/lang}{/capture}

{capture assign='contentTitle'}{lang}wcf.acp.quiz.question.{@$action}{/lang}{/capture}
...
<form id="formContainer" method="post" action="{if $action == 'add'}{link controller='QuizQuestionAdd'}{/link}{else}{link controller='QuizQuestionEdit' id=$questionObj->questionID}{/link}{/if}">
	<div class="section">
...
<script data-relocate="true">
	require(['Language', 'UZ/Quiz/UploadQuestionImage'], function(Language, UZQuizUploadQuestionImage) {
		Language.addObject({
			'wcf.acp.quiz.quiz.image.error.invalidExtension': '{jslang}wcf.acp.quiz.quiz.image.error.invalidExtension{/jslang}'
		});
		new UZQuizUploadQuestionImage({if $action == 'add'}0{else}{@$questionObj->questionID}{/if}, '{$tmpHash}');
	});
</script>
